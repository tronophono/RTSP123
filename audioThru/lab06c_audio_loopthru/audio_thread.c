...
//
    ...
    while( !envPtr->quit )
    {
      ...
      while (snd_pcm_writei(pcm_output_handle, inputBuffer, blksize/BYTESPERFRAME) < 0) {
        snd_pcm_prepare(pcm_output_handle);
        ERR( "<<<<<<<<<<<<<<< Buffer Underrun >>>>>>>>>>>>>>>\n");
//        status = AUDIO_THREAD_FAILURE;
//        goto cleanup;
	// Send out an extra blank buffer if there is an underrun and try again.
	    memset(inputBuffer, 0, blksize);		// Clear the buffer
	    snd_pcm_writei(pcm_output_handle, inputBuffer, exact_bufsize);
      }
	DBG("%d, ", count++);
    }
    DBG("\n");
